...
## Investment Book · Parte I · 🟢 **CERRADO v2** · aprobado el 13 de agosto de 2026 *(D82)*
...
**Caso 1 — Caso estudiado durante la construcción: un fondo flexible de renta fija.** Un
fondo de renta fija de gestión activa, con mandato amplio *(puede moverse entre tipos de
deuda, plazos y niveles de riesgo)*. La tentación es clasificarlo automáticamente «porque
es renta fija». El árbol no lo permite: la pregunta 2 no dice *«¿es renta fija?»* sino
*«¿su función dominante es estabilizar la cartera o responder a escenarios distintos de
los activos productivos?»*. La respuesta exige mirar la **tesis**: el fondo se estudió
para dar al Freno una fuente de renta y comportamiento distinta del monetario, dentro de
la función estabilizadora del módulo. Respuesta: sí → **Freno**, con forma de gestión
*activa delegada* y una anotación en la ficha: su mandato es más amplio que el del
vehículo al que sustituiría, y esa amplitud es un riesgo que se vigila *(capítulo 11)*.
**Aunque el vehículo pueda ser sustituido, la enseñanza metodológica permanece: primero se
define la función; después se elige el producto que mejor la cumple.**
...
probó y se descartó porque **el análisis cuestionó que aportara suficiente diferenciación
respecto al núcleo** *(las cifras se demuestran en el capítulo 14)*. La clasificación
habría sido posible; la posición, cuestionada. **Clasificar
...
En la versión de referencia utilizada para este capítulo, el protocolo no ha hecho falta:
todas las posiciones tienen función dominante clara. Ese dato también informa — la cartera se construyó con
...
|---|---|
| **ISIN** | El mismo producto cotiza en varias bolsas con distintos códigos de pizarra. En este proyecto, más de un vehículo aparece en dos o tres mercados. **El ISIN identifica de forma robusta el valor o la clase concreta; el ticker y el mercado completan la identificación de la línea de negociación** |
| **Clase de participación** | Dos posiciones de la cartera se contrataron en una clase que las herramientas de análisis no reconocían; el análisis se hizo con otra clase del **mismo fondo**. Las clases **pueden compartir estrategia y cartera subyacente, pero tener costes y características diferentes — incluida, en algunos casos, la cobertura de divisa**. Sin este campo, esas diferencias se vuelven invisibles y el coste publicado sale mal |
...
| **Cobertura de divisa** | **Durante la conciliación apareció una diferencia de cobertura de divisa entre vehículos analizados. El caso demostró que dos productos aparentemente similares pueden tener comportamientos distintos si uno cubre la divisa y otro no; por eso la cobertura pasó a ser un campo obligatorio** |
...
> acierta por nosotros. Juntas hacen mucho más difícil tomar una decisión sin dejar un
> rastro que permita revisarla después.**
...
| Fecha | Acción |
|---|---|
| 2026-08-13 | **Borrador v1 completo del capítulo 7** — 8 secciones. Título en llano *(«las herramientas del método»)*; Gate → control final de aceptación; Protocolo de Extensión citado una vez como nombre formal. Tres casos reales del árbol *(fondo flexible del Freno, calidad, cobre)*, seis campos del modelo de datos con el problema real que los originó, y las cuatro limitaciones exigidas — incluida la comparabilidad inter-analista como no testada. **Ninguna cifra exacta publicada**; las del caso calidad remitidas al cap. 14. Auditoría superada |
| 2026-08-13 | 🟢 **CERRADO v2 (D82).** Aplicadas las 8 microcorrecciones: repetibilidad como «el mismo procedimiento puede volver a ejecutarse y deja un registro comparable»; el fondo flexible como caso estudiado durante la construcción, con la enseñanza que permanece aunque el vehículo cambie; la calidad como «el análisis cuestionó que aportara suficiente diferenciación»; ISIN + ticker/mercado como identificación completa; las clases «pueden compartir estrategia pero tener costes y características diferentes»; la cobertura de divisa como enseñanza general de la conciliación, sin fijar qué vehículo estaba cubierto; el protocolo referido a «la versión de referencia utilizada para este capítulo»; síntesis con «mucho más difícil», no «imposible». La v1 se conserva como trazabilidad |
