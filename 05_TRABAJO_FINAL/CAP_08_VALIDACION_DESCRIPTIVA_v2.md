...
## Investment Book · Parte I · 🟢 **CERRADO provisionalmente (v2)** · 13 de agosto de 2026 — *sujeto únicamente a las dos citas pendientes (Browne, Swensen)*
...
— verificada]** Como referencia se cita el *Vanguard Balanced Index Fund*: **una
implementación institucional contemporánea de la lógica 60/40**, estructurada
aproximadamente con esa distribución. ⚠️ **No es el origen del 60/40 ni «la» cartera
60/40** — la proporción es anterior y de autoría difusa; el fondo solo demuestra que la
lógica sigue implementándose institucionalmente hoy.
...
**Qué se pierde o simplifica.** El contexto del mandato. **Era una instrucción concreta
para la gestión del patrimonio destinado a su esposa — no una recomendación universal para
cualquier inversor.** Traducida a dos casillas parece una receta general, y Buffett no la
...
**Qué se pierde o simplifica.** **La traducción modular puede perder restricciones y
convicciones específicas de Swensen sobre los activos adecuados para el inversor
particular** — su preferencia por determinados tipos de renta fija es parte del diseño
original, y el módulo Freno de PMMA, más amplio, no la conserva por sí solo. ⚠️ *La
formulación exacta de esas restricciones queda pendiente de la cita primaria de*
Unconventional Success *(página por localizar)*; hasta entonces no se citan como
...
jerarquía del autor, para quien la señal táctica es el corazón del modelo. Segunda: el
efectivo que genera la señal táctica. Que quede clarísimo: **ese efectivo NO es Reserva
Operativa PMMA** — no es caja de ejecución esperando una compra; **es parte de la propia
estrategia táctica**, un estado dictado por la señal. Tampoco es Freno estratégico, porque
no es permanente. El caso demuestra algo importante para todo el método: **el mismo activo
físico —el efectivo— puede cumplir funciones completamente distintas** según el papel que
desempeñe *(caja de ejecución · estabilizador permanente · posición táctica)*. La casilla
...
profundamente distintas pueden leerse con las mismas siete columnas.** ⚠️ Sobre las
columnas vacías, precisión importante: un guion en Defensivos o Asimetría **no afirma que
esas exposiciones «no existan»** dentro de los fondos de cada cartera — significa que,
**en la traducción por función dominante, no requieren una asignación independiente a ese
módulo.** Las grandes compañías defensivas de un índice global, por ejemplo, viven dentro
del Motor.
...
1. **No hacen falta los siete módulos.** Dos carteras funcionan con dos; en ninguna de
   las ocho traducciones Defensivos o Asimetría requieren asignación independiente —
   funciones que nuestra implementación sí separa, y esa diferencia también describe algo.
...
> **PMMA puede describir de forma compacta arquitecturas construidas con filosofías muy
> distintas, pero la traducción pierde parte de la lógica original de algunos diseños.
> Funciona mejor como lenguaje de organización y comparación que como sustituto de la
> filosofía que originó cada cartera.** Donde la traducción simplifica o pierde algo,
> queda dicho caso por caso — y este capítulo no reclama universalidad, ni superioridad,
> ni validación estadística, ni capacidad predictiva.
...
| Fecha | Acción |
|---|---|
| 2026-08-13 | **Borrador v1 completo del capítulo 8** — 12 secciones. Los ocho casos con la estructura obligatoria de cinco pasos, **incluida la columna «qué se pierde o simplifica» en los ocho** *(las dos pérdidas mayores: el equilibrio de riesgo de All Seasons y la jerarquía táctica de GTAA; más la tensión clasificatoria de la liquidez táctica)*. Matriz documental aplicada: 6 fuentes verificadas, 2 composiciones validadas con **página pendiente y no inventada** *(Browne, Swensen)*. Sección explícita de qué NO demuestra el ejercicio. Auditoría superada |
| 2026-08-13 | 🟢 **CERRADO provisionalmente (v2).** Auditoría A-G aplicada: 60/40 como *implementación institucional contemporánea de la lógica 60/40*; Buffett como instrucción para el patrimonio de su esposa, no recomendación universal; All Seasons/All Weather ya distinguidos; Swensen sin la palabra «prohibición» — *la traducción puede perder restricciones y convicciones específicas*, con la cita primaria pendiente; **el efectivo táctico de GTAA NO es Reserva Operativa PMMA: es parte de la estrategia, y el caso demuestra que el mismo activo físico puede cumplir funciones completamente distintas**; las columnas vacías de la tabla reformuladas *(no requieren asignación independiente ≠ no existen)*; conclusión sustituida por la formulación aprobada. **Sujeto únicamente a las páginas de Browne y Swensen** |
