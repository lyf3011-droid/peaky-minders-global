...
El capítulo 4 fijó el criterio de falsación organizativo. Su formulación comparativa
exacta es ésta:

> **PMMA solo justifica su mayor complejidad si, frente a un sistema convencional más
> sencillo, aporta una mejora material en trazabilidad, reproducibilidad del
> procedimiento, detección de duplicidades, separación función/vehículo, gobernanza y
> auditabilidad.**

⚠️ El criterio **no exige que PMMA «encuentre siempre problemas»**: una cartera sana puede
no tener duplicidades que detectar, y eso no falsa el método — lo que lo falsaría es no
superar a la alternativa sencilla en las seis dimensiones cuando se comparan. La lista
operativa: **consideraríamos que PMMA no añade suficiente valor si se diera cualquiera de
estas situaciones de forma sostenida:**
...
3. **El método no supera a la alternativa sencilla** — comparado con una clasificación
   convencional por clases de activo, no aporta mejora material en ninguna de las seis
   dimensiones del criterio *(trazabilidad, reproducibilidad, detección de duplicidades
   cuando existen, separación función/vehículo, gobernanza, auditabilidad)*.
...
| 2026-08-14 | **v2 — auditoría externa aplicada.** Criterio de utilidad/falsación reescrito en forma **comparativa** con la formulación aprobada; eliminada la exigencia implícita de que el método «encuentre siempre problemas» *(condición 3 reformulada)*. La v1 se conserva |
